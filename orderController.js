...
exports.updateOrderStatus = async (req, res) => {
  try {
    const { id } = req.params;
    const { status, trackingNumber, shippingProvider, paymentStatus } = req.body;
    ...
    if (paymentStatus) {
      const allowedPaymentStatuses = ['pending', 'payment_initiated', 'authorized', 'captured', 'paid', 'failed', 'cancelled', 'expired', 'refund_pending', 'refunded'];
...
exports.handleReturnRequest = async (req, res) => {
  try {
    ...
    const {
      action,
      refundAmount,
      replacementItemOutOfStock,
      upiId,
      adminNotes,
      orderStatus
    } = req.body;

    const normalizedAction = action === 'approved' || action === 'approve' ? 'approve' : action === 'rejected' || action === 'reject' ? 'reject' : action;
    ...
    if (normalizedAction === 'approve') {
      ...
      order.returnRequest.replacementItemOutOfStock = Boolean(replacementItemOutOfStock ?? order.returnRequest.replacementItemOutOfStock);
      order.returnRequest.adminNotes = adminNotes || order.returnRequest.adminNotes;

      if (upiId) {
        order.returnRequest.manualRefundUpi = {
          upiId,
          maskedUpiId: upiId.replace(/(?<=.{3}).(?=.*@)/g, '*'),
          adminOnly: true
        };
      }

      order.status = orderStatus || 'Replacement Request Approved';
      order.paymentStatus = order.returnRequest.replacementItemOutOfStock ? 'refunded' : order.paymentStatus;
    } else if (normalizedAction === 'reject') {
      order.returnRequest.status = 'rejected';
      order.returnRequest.rejectedAt = new Date();
      order.returnRequest.adminNotes = adminNotes || order.returnRequest.adminNotes;
      order.status = orderStatus || 'Replacement Request Rejected';
    } else {
    ...
    res.json({ success: true, message: `Replacement request ${normalizedAction}d`, data: order });
  } catch (error) {
